...
func ReNode(g graph.Graph, id int, exFlag *bool, t uint32, prNo int ) {
    node, _ := g.GetNode(id)
    ...
    for {
		...
		CheckError(err)
		//log.Println("#",id, " Received ", string(buf[0:n]), " from ", addr, " ", n, " bytes")
...
func RootNode(g graph.Graph, id int, exFlag *bool, t uint32, prNo int, BsBig *int) {
    node, _ := g.GetNode(id)
	neighbors, _ := g.Neighbors(id)
...
func main() {
    ...
    nPtr := flag.Int("n", 100, "number of nodes")
    n := *nPtr
	g := graph.Generate(n, 10, 20, 25000)
	flag := false
    x := 0
	for i := 1; i < n; i++ {
		go ReNode(g, i,  &flag, 1, n)
	}
	
	go RootNode(g, 0, &flag, 1, n, &x)
